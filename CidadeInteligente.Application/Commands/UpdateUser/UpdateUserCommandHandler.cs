...
public class UpdateUserCommandHandler(IUnitOfWork unitOfWork) : IRequestHandler<UpdateUserCommand, Unit>
{
    ...
    {
        User user = await _unitOfWork.Users.GetByIdAsync(request.UserId, true) ?? throw new UserNotExistException();

        if (await _unitOfWork.Users.IsEmailInUseExceptByUserId(request.Email, user.UserId))
